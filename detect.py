...
model_f1 = tf.keras.models.load_model("/content/yolov5/my_model2.h5")
tabla_switch = {
    0: 'CVEL_90',
    1: 'CVEL_100',
    2: 'CVEL_15',
...
def run(
    ...
    for path, im, im0s, vid_cap, s in dataset:
       ...
       #pred.shape
        ...
        #x = list(pred.size()) 
        #print(x)
        #print(torch.shape(pred))
       
        # Second-stage classifier (optional)
        #y=np.array(pred , dtype = np.uint8)
        #print(y)
        #print(im0s.shape)
        #im0s = im0s.reshape(im0s.shape[1], im0s.shape[2], im0s.shape[0])
        #cropped_image = image[start_y:start_y+height, start_x:start_x+width]
        #plt.imshow(im0s)
        #plt.show()
        #plt.imsave('saved_image.png', im0s)
        #print(im0s.shape)
        #image = cv2.imread(imu)
        #pred = utils.general.apply_classifier(pred, model_f1, im, im0s)
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_crop:
                        ...
                        Prediciones=model_f1.predict(curImg)
                        #print(Prediciones)
                        predicted_class = np.argmax(Prediciones)
                        #print(predicted_class)
                        #print(Prediciones[0,0])
                        #print(f"The predicted class is: {predicted_class}")
                        #print(usa_switch(predicted_class))
                        #print(names[c],Prediciones[0,predicted_class])
                        po=po+1
                        label1 = None if hide_labels else (usa_switch(predicted_class) if hide_conf else f'{usa_switch(predicted_class)} {Prediciones[0,predicted_class]:.2f}')
